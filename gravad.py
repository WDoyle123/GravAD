...
def psd_func(conditioned):
    ...
    Returns:
    jax.numpy.array: Power spectral density frequency series array and the inverse
    """
    psd = conditioned.psd(4)
    psd = interpolate(psd, conditioned.delta_f)
    psd = inverse_spectrum_truncation(psd, int(4 * conditioned.sample_rate), low_frequency_cutoff = 15)
    return jnp.array(psd), 1 / jnp.array(psd)
...
def preprocess(event, detector, simulated_signal):

    if isinstance(event, str) and event.startswith('GW'):
        merger = Merger(event)
        strain = merger.strain(detector) * 1e22
        ...
        fdata = conditioned.to_frequencyseries()
        delta_f = conditioned.delta_f
        fdata = jnp.fft.fft(simulated_signal)
        fdata_jax = jnp.array(fdata)

    # Calculate the PSD of the conditioned data
    psd_jax, inverse_psd = psd_func(conditioned)

    return fdata_jax, delta_f, psd_jax, inverse_psd, conditioned

def compile():

    print("Beginning Compilation...")
    fdata_jax, delta_f, psd_jax, _, _  = preprocess("GW150914", "H1", 0)
    chi1 = 0.
    chi2 = 0.
    tc = 1.0
    phic = 1.
...
def analyse_signal(fdata_jax, psd_jax, freqs, delta_f, event, strain):
    rng_key = random.PRNGKey(SEED)
    init_mass1, init_mass2, rng_key = gen_init_mass(rng_key)
    ...
    for i in range(max(iterations)):
        results[i] = {'snr': float(snr[i]), 'mass1s': float(mass1[i]), 'mass2s': float(mass2[i]), 'combined_mass': float(mass1[i] + mass2[i]), 'iters': int(iterations[i])}

    max_snr = get_max_snr_array(results, event, strain, total_time)
    print(f"Time Taken: {total_time:.2f}, Template:{iterations[-1]}, SNR: {max_snr['snr']}\n")
    return results, max_snr

def simulated_signals():
    folder_path = 'simulated_signals'
        ...
        if filename.endswith('.csv'):
            ...
            with open(file_path, 'r') as file:
                ...
                for row in reader:
                    ...
                    float_array.extend(float_values)
            float_array = jnp.asarray(float_array)

            print(f"Analysing Simulated Signal: {filename}")
            fdata_jax, delta_f, psd_jax, inverse_psd, conditioned = preprocess(0, 0, float_array)
            freqs = frequency_series(delta_f)
            
            pattern = r'(\d+)_(\d+)_'
            match = re.search(pattern, filename)
            combined_mass= '_'.join(match.groups())

            results, max_snr = analyse_signal(fdata_jax, psd_jax, freqs, delta_f, "simulated_signal", combined_mass)

            filename1 = f"simulated_results_for_{filename}_T_{TEMPERATURE:.2f}_AR_{ANNEALING_RATE:.3f}_MI_{MAX_ITERS}_{LRL}_{LRU}_SEED{SEED}.json"
            with open(f"simulated_results/results/{filename1}", "w") as f:
                json.dump(results, f)
                
            filename2 = f"simulated_max_snr_for_{filename}_T_{TEMPERATURE:.2f}_AR_{ANNEALING_RATE:.3f}_MI_{MAX_ITERS}_{LRL}_{LRU}_SEED{SEED}.json"
            with open(f"simulated_results/max_snr/{filename2}", "w") as f:
                json.dump(max_snr, f)
...
def real_signals():
    for event in EVENTS:
        for strain in STRAINS:
             ...
             fdata_jax, delta_f, psd_jax, inverse_psd, conditioned = preprocess(event, strain, 0)
             freqs = frequency_series(delta_f)
             results, max_snr = analyse_signal(fdata_jax, psd_jax, freqs, delta_f, event, strain)

             filename = f"real_results_for_event_{event}_strain_{strain}T_{TEMPERATURE:.2f}_AR_{ANNEALING_RATE:.3f}_MI_{MAX_ITERS}_{LRL}_{LRU}_SEED{SEED}.json"
             with open (f"real_results/results/{filename}", "w") as f:
                 json.dump(results, f)
             filename = f"max_snr_for_event_{event}_strain_{strain}T_{TEMPERATURE:.2f}_AR_{ANNEALING_RATE:.3f}_MI_{MAX_ITERS}_{LRL}_{LRU}_SEED{SEED}.json"
             with open (f"real_results/max_snr/{filename}", "w") as f:
                 json.dump(max_snr, f)
def plotter(signal_type):
    import plots
    if signal_type == "simulated_signal":
        folder_results = "simulated_results/results/"
        folder_max = "simulated_results/max_snr/"
        for filename in os.listdir(folder_results):
            if filename.endswith('.json'):
                result_file_path = os.path.join(folder_results, filename)
                max_snr_file_path = os.path.join(folder_max, filename.replace("results", "max_snr"))
                if os.path.isfile(max_snr_file_path):
                    with open(result_file_path, 'r') as result_file, open(max_snr_file_path, 'r') as max_snr_file:
                        results = json.load(result_file)
                        max_snr = json.load(max_snr_file)
                        event_name = max_snr['event_name']
                        strain = max_snr['strain']
                        total_time = max_snr['time']
                
                plots.plot_snr_vs_mass(results, max_snr)
                plots.plot_snr_vs_iteration(results, max_snr)
    #plot_snr_timeseries(results, freqs, params, fdata_jax, psd_jax, delta_f)

def main():
    compile()
    if False:
        real_signals()
        folder_results = "real_results/results/"
        for filename in os.listdir(folder_results):
            if filename.endswith('.csv'):
                file_path = os.path.join(folder_results, filename)
                with open(file_path, 'r') as file:
                    reader = csv.reader(file)
                    for row in reader:
                        float_values = [float(value) for value in row]
